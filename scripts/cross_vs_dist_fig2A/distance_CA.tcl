...
proc interaction_time {sel sel2 molid_sel frames_end dt filename} {
	...
	for { set f1 0} { $f1 < $frames_end } { incr f1 $dt } {
			...
			create_file $filename $list_dist 
			
			$sel_1 delete
			$sel_2 delete
        }
	#set plothandle [multiplot -x $list_frame -y $list_dist -title "Crossing Angle vs time" -lines -linewidth 3 -marker point -plot]
...
proc cross_vs_time {molid sel1 sel2 start end dt filename} {
	...
	#exec python /data/VERITY/script/histo_cross.py $filename "-80" "80" $filename ".svg"
...
for { set nb 1 } { $nb < 31 } { incr nb 1 } {
	...
	set data_name_dist [join [concat "md.flrt2TMH_jx_martini_DPPC_$nb" "_dist.dat"] ""]
	...
	cross_vs_time $molid "index 8 to 59 and type BB" "index 80 to 131 and type BB" 0 $frames_end 1 $data_name
	interaction_time "index 15 16 21 22" "index 87 88 93 94" $molid $frames_end 1 $data_name_dist 
}
